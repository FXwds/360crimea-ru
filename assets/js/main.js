...
//Fullscreen режим
let Fullscreen = {
    launch: (element) => element.mozRequestFullScreen ? element.mozRequestFullScreen() : element.webkitRequestFullscreen ? element.webkitRequestFullscreen() : element.msRequestFullscreen ? element.msRequestFullscreen() : element.requestFullscreen(),
    exit: () => document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.webkitExitFullscreen ? document.webkitExitFullscreen() : document.msExitFullscreen ? document.msExitFullscreen() : document.exitFullscreen()
};
...
function addDayNight(img, display) {
	...
	else if(imgName == 'nav_up_winter'){
		$('#day-night-btn').append('<span>' + translation.Winter_Mode(lang) + '</span>');
	}
	else if(imgName == 'nav_up_summer'){
		$('#day-night-btn').append('<span>' + translation.Summer_Mode(lang) + '</span>');
	}
...
function appendMap(withPlaceId) {
    ...
    $('.back').attr('data-id', 4).attr('data-search-id', 4);
...
function OnSearch() {
    ...
    var categoryId = parseInt($('.search-logo').attr('data-category'));
            ...
            if ($('#map-search-btn').hasClass('active')) {
                ...
                if (categoryId == 0) {
                    GetTopObjects();
										...
										if(parseInt($('.search-logo').attr('data-iscompanies')) == 1){
												GetCompanies(parseInt($('.search-logo').attr('data-id')));
										}
										else {
...
function GetParentCategories() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-parent-categories',
        data: {
            lang: cultureKey,
			rand: getRandom()
        },
...
function GetFilterMap() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-filter-map',
        data: {
            lang: cultureKey,
            type: type,
            rentType: rentType,
...
function GetTopObjects() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-top-objects',
        data: {
            lang: cultureKey,
			rand: getRandom()
        },
...
function OnFilterSearch() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-real-estate-objects',
        data: {
            categoryId: categoryId,
            lang: cultureKey,
            type: type,
            ...
            if (type == 1) {
                    ...
                    if (rentType == 1 && categoryId != '327-land') {
                        if (parseInt(item.priceRentDaily) >= priceNowMin && parseInt(item.priceRentDaily) <= priceNowMax && parseInt(item.priceRentDaily) != 0 && (count <= 10) && (districts == 0 || districts.indexOf(item.district) >= 0)) {
                            $container.append('<div class="sisea-result estate offset">\
															<a href="#!p=' + item.itemId + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.squareImg + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? (item.type == 1 ? '<div class="rent"></div>' : '') + (item.type == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.itemId + '</div><p>' + item.title + '</p></a>\
															<div class="object-description">\
															<p class="card-object-location"><i></i>' + item.location + '</p>\
															<p class="card-object-category"><i></i>' + item.category + '</p>' + (item.categoryId != '327-land' ? '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>' : '') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + '<p class="object-sea-view">' + (item.seaView == 'true' ? 'Вид на море' : '') + '</p>' + (item.pool == 0 ? '' : '<p class="card-pool">' + (item.pool == 1 ? 'Приватный бассейн' : 'Community pool') + '</p>') + (item.type == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentDaily) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '<p class="object-price">' + priceToString(item.priceRentDaily) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>') + '</div>\
                ...
                for (key in items) {
                    ...
                    if ((parseInt(item.priceSale) >= priceNowMin) && (parseInt(item.priceSale) <= priceNowMax) && (count <= 10) && (districts == 0 || districts.indexOf(item.district) >= 0)) {
                        $container.append('<div class="sisea-result estate offset">\
															<a href="#!p=' + item.itemId + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.squareImg + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? (item.type == 2 ? '<div class="sale"></div>' : '') + (item.type == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.itemId + '</div><p>' + item.title + '</p></a>\
															<div class="object-description">\
															<p class="card-object-location"><i></i>' + item.location + '</p>\
...
function GetHotRealEstates() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-hots-real-estates',
        data: {
            lang: cultureKey,
            currency: $('#currency-select .select-active').attr('data-currency'),
			rand: getRandom()
        ...
        success: function(data) {
            ...
            $('.search-logo').append('<svg version="1.1" id="Layer_2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"\
								...
								<i style="background-image: url(/assets/images/category-icons/R_Panel_Icon_Real_Estate.svg);"></i>').css('background-image', '').attr('data-id', 41).attr('data-category', '41-real-estate').attr('data-description', data.parentDescription).attr('data-url', data.parentUrl).attr('data-image', data.parentImage).attr('data-isfolder', 0);
            $('#search').attr('placeholder', translation.Search(cultureKey));
            $('.back').attr('data-id', parentId).attr('data-search-id', parentId);
            var $container = searchResults.append('<div class="category-list"></div>').find('div.category-list');
            for (key in hots) {
                ...
                $container.append('<div class="sisea-result estate">\
															...
															<div class="object-description">\
															<p class="card-object-location"><i></i>' + item.location + '</p>\
															<p class="card-object-category"><i></i>' + item.category + '</p>' + (item.categoryId == '327-land' ? '' : '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + '<p class="object-sea-view">' + (item.seaView == 'true' ? 'Вид на море' : '') + '</p>' + (item.pool == 0 ? '' : '<p class="card-pool">' + (item.pool == 1 ? 'Приватный бассейн' : 'Community pool') + '</p>') + (item.sold == 'true' ? '<p class="object-price">Sold</p>' : (item.type == 1 ? '<p class="object-price">' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '<text> monthly+</text></p>' : (item.type == 2 ? '<p class="object-price">' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : (item.type == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '')))) + '</div>\
															<div class="contact-object-btn"><img src="/assets/images/object_contact_icon.svg"/><span>' + translation.Contact(cultureKey) + '</span></div>\
															<div class="favorite-object-btn">22<img src="/assets/images/favorites_icon.svg"/><span>' + translation.Favorites(cultureKey) + '</span></div>\
									</div>');
                $.cookie('object_' + item.itemId, true, {
                    expires: 300,
                    path: '/'
...
function GetCompanies(itemId){
	var searchResults = $('.site-search-results .search-content');
	var isCompanies = $('.search-logo').attr('data-iscompanies');
	var cultureKey = getUrlVars()["lang"];
	if(typeof cultureKey === 'undefined'){
		cultureKey = 'en';
	}
	$.ajax({
        url: 'https://360crimea.ru/api/get-companies',
        data: {
            itemId: itemId,
						lang: cultureKey,
						offset: 0,
      ...
      var parentCategory = data.parentCategory;
			$('.search-logo').attr('data-id', itemId).attr('data-category', parentId).attr('data-categoryid', parentId);
			$('.back').attr('data-id', parseInt(parentId)).attr('data-search-id', parentId);
			var $container = searchResults.append('<div class="category-list"></div>').find('div.category-list');
                for (key in items) {
                    var item = items[key];
                    $container.append('<div class="sisea-result slide offset">\
										<a href="#!p=' + (isCompanies == 1 ? itemId : item.itemId) + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.image + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? '' : '<div class="new"></div>') + '<p>' + item.titleWithLocation + '</p>\
...
function OnCategorySearch(category, categoryId, isFolder, title) {
    if (categoryId == 41) {
        ...
        var url = '';
        if (!isFolder) {
            url = 'https://360crimea.ru/api/objects';
        } else {
            url = 'https://360crimea.ru/api/categories';
        }
        $.ajax({
            url: url,
            data: {
            ...
            success: function(data) {
                ...
                if (!isFolder) {
                    ...
                    for (key in items) {
                        var item = items[key];
                        $container.append('<div class="sisea-result slide offset">\
													<a href="#!p=' + item.itemId + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.image + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? '' : '<div class="new"></div>') + '<p>' + item.titleWithLocation + '</p>\
													</a>' + (item.video != '0' ? '<button class="youtube-btn" data-code="' + item.video + '"></button>' : '') + '</div>');
                        $.cookie('object_' + item.itemId, true, {
                            expires: 300,
                            path: '/'
                    ...
                    for (key in items) {
                        ...
                        $container.append('<li class="' + item.catClass + '">\
											<a data-id="' + item.itemId + '" data-search-id="' + item.searchId + '" title="' + item.title + '">\
												<span>\
													<i style="background-image: url(' + (categoryId == 4 ? item.icon : item.image) + ');"></i>\
												</span>\
												<p>' + item.title + '</p>\
											</a>\
										</li>');
...
function OnLoadObjects(category, offset) {
    ...
    var url = 'https://360crimea.ru/api/loading-objects';
    ...
    var exclude = '';
		var isComanies = parseInt($('.search-logo').attr('data-iscompanies'));
		...
		if (category == 41) {
			if (isFilter) {
				...
				exclude = GetExludeObjects(false);
					...
					url = 'https://360crimea.ru/api/get-all-real-estates';
			}
		}
		if (category == 0) {
			url = 'https://360crimea.ru/api/get-top-objects';
                    ...
                    if (type == 1) {
                            ...
                            if (rentType == 1 && categoryId != '327-land') {
                                if (parseInt(item.priceRentDaily) >= priceNowMin && parseInt(item.priceRentDaily) <= priceNowMax && parseInt(item.priceRentDaily) != 0 && (districts == 0 || districts.indexOf(item.district) >= 0)) {
                                    searchResults.append('<div class="sisea-result estate offset">\
																	<a href="#!p=' + item.itemId + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.squareImg + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? (item.type == 1 ? '<div class="rent"></div>' : '') + (item.type == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.itemId + '</div><p>' + item.title + '</p>' + (item.sold == 'true' ? '<div class="sold"></div>' : '') + '</a>\
																	<div class="object-description">\
																	<p class="card-object-location"><i></i>' + item.location + '</p>\
																	<p class="card-object-category"><i></i>' + item.category + '</p>' + (item.categoryId != '327-land' ? '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>' : '') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + '<p class="object-sea-view">' + (item.seaView == 'true' ? 'Вид на море' : '') + '</p>' + (item.pool == 0 ? '' : '<p class="card-pool">' + (item.pool == 1 ? 'Приватный бассейн' : 'Community pool') + '</p>') + (item.type == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentDaily) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '<p class="object-price">' + priceToString(item.priceRentDaily) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>') + '</div>\
																	<div class="contact-object-btn"><img src="/assets/images/object_contact_icon.svg"/><span>' + translation.Contact(cultureKey) + '</span></div>\
                        ...
                        for (key in items) {
                            ...
                            if (parseInt(item.priceSale) >= priceNowMin && parseInt(item.priceSale) <= priceNowMax && (districts == 0 || districts.indexOf(item.district) >= 0)) {
                                searchResults.append('<div class="sisea-result estate offset">\
															<a href="#!p=' + item.itemId + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.title + '" style="background-image: url(' + item.squareImg + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? (item.type == 2 ? '<div class="sale"></div>' : '') + (item.type == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.itemId + '</div><p>' + item.title + '</p>' + (item.sold == 'true' ? '<div class="sold"></div>' : '') + '</a>\
															<div class="object-description">\
															<p class="card-object-location"><i></i>' + item.location + '</p>\
															<p class="card-object-category"><i></i>' + item.category + '</p>' + (item.categoryId != '327-land' ? '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>' : '') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + '<p class="object-sea-view">' + (item.seaView == 'true' ? 'Вид на море' : '') + '</p>' + (item.pool == 0 ? '' : '<p class="card-pool">' + (item.pool == 1 ? 'Приватный бассейн' : 'Community pool') + '</p>') + (item.type == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '<p class="object-price">' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>') + '</div>\
															<div class="contact-object-btn"><img src="/assets/images/object_contact_icon.svg"/><span>' + translation.Contact(cultureKey) + '</span></div>\
                    ...
                    for (key in items) {
                        ...
                        if (item.type == 1 || item.type == 2 || item.type == 3) {
                            searchResults.append('<div class="sisea-result estate search offset">\
											...
											<div class="object-description">\
											<p class="card-object-location"><i></i>' + item.location + '</p>\
											<p class="card-object-category"><i></i>' + item.category + '</p>' + (item.categoryId == '327-land' ? '' : '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + (item.sold == 'true' ? '<p class="object-price">Sold</p>' : (item.type == 1 ? '<p class="object-price">' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '<text> monthly+</text></p>' : (item.type == 2 ? '<p class="object-price">' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : (item.type == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '')))) + '</div>\
									</div>');
                        } else {
                    searchResults.append('<div class="sisea-result slide offset">\
											<a ' + (item.objectLink.length < 10 ? 'href="#!p=' + (isComanies == 1 ? category : item.itemId)  + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '"' : 'href="' + item.objectLink + '" target="_blank"') + ' title="' + item.title + '" style="background-image: url(' + item.image + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.itemId + '">' + ($.cookie('object_' + item.itemId) ? (item.type == 1 ? '<div class="rent"></div>' : '') + (item.type == 2 ? '<div class="sale"></div>' : '') + (item.type == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + (item.type == 3 || item.type == 2 || item.type == 1 ? '<p>' + item.titleWithCategory + '</p>' : (category == 0 ? '<p class="big-title">' + item.titleWithLocation + '</p>' : '<p>' + item.titleWithLocation + '</p>')) + (item.sold == 'true' ? '<div class="sold"></div>' : '') + '</a>' + (item.video != '0' ? '<button class="youtube-btn" data-code="' + item.video + '"></button>' : '') + '</div>');
                        }
                        $.cookie('object_' + item.itemId, true, {
                            expires: 300,
                            path: '/'
...
function OnAjaxSearch(title, OnLoaded, count, exclude) {
    ...
    var currency = $('#currency-select .select-active').attr('data-currency');
    $.ajax({
        url: 'https://360crimea.ru/api/ajax-search',
        data: {
            lang: cultureKey,
            title: title,
						offset: count,
        ...
        success: function(data) {
            ...
            for (key in items) {
				...
				if(item.template == 4){
					...
					if (categoryId == '41-real-estate') {
						$container.append('<div class="sisea-result estate offset">\
																<a href="#!p=' + item.id + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.pagetitle  + '" style="background-image: url(' + item.square_img + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.id + '">' + ($.cookie('object_' + item.id) ? (item.typeRealEstate == 1 ? '<div class="rent"></div>' : '') + (item.typeRealEstate == 2 ? '<div class="sale"></div>' : '') + (item.typeRealEstate == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.id + '</div> <p>' + item.pagetitle + '</p></a>\
																<div class="object-description">\
																<button>rent</button>\
																<p class="card-object-location"><i></i>' + item.districtTitle + '</p>\
																<p class="card-object-category"><i></i>' + item.categoryTitle + '</p>' + (item.categoryId == '327-land' ? '' : '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + '<p class="object-sea-view">' + (item.seaView == 'true' ? 'Вид на море' : '') + '</p>' + (item.pool == 0 ? '' : '<p class="card-pool">' + (item.pool == 1 ? 'Приватный бассейн' : 'Community pool') + '</p>') + (item.sold == 'true' ? '<p class="object-price">Sold</p>' : (item.typeRealEstate == 1 ? '<p class="object-price">' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : (item.typeRealEstate == 2 ? '<p class="object-price">' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : (item.typeRealEstate == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '')))) + '</div>\
																<div class="contact-object-btn"><img src="/assets/images/object_contact_icon.svg"/><span>' + translation.Contact(cultureKey) + '</span></div>\
																<div class="favorite-object-btn"><img src="/assets/images/favorites_icon.svg"/><span>' + translation.Favorites(cultureKey) + '</span></div>\
										</div>');
					} else {
						if (item.typeRealEstate == 1 || item.typeRealEstate == 2 || item.typeRealEstate == 3) {
							$container.append('<div class="sisea-result estate search offset">\
																<a href="#!p=' + item.id + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.pagetitle + '" style="background-image: url(' + item.square_img + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.id + '">' + ($.cookie('object_' + item.id) ? (item.typeRealEstate == 1 ? '<div class="rent"></div>' : '') + (item.typeRealEstate == 2 ? '<div class="sale"></div>' : '') + (item.typeRealEstate == 3 ? '<div class="sale-rent"></div>' : '') : '<div class="new"></div>') + '<div class="object-id">Id: ' + item.id + '</div> <p>' + item.pagetitle + '</p>' + (item.sold == 'true' ? '<div class="sold"></div>' : '') + '</a>\
																<div class="object-description">\
																<p class="card-object-location"><i></i>' + item.districtTitle + '</p>\
																<p class="card-object-category"><i></i>' + item.categoryTitle + '</p>' + (item.categoryId == '327-land' ? '' : '<p class="card-bedrooms">' + item.bedrooms + '<i></i></p>') + '<p class="card-area">' + item.area + ' <span>кв. м.</span></p>' + (item.sold == 'true' ? '<p class="object-price">Sold</p>' : (item.typeRealEstate == 1 ? '<p class="object-price">' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '<text> monthly+</text></p>' : (item.typeRealEstate == 2 ? '<p class="object-price">' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : (item.typeRealEstate == 3 ? '<p class="second-object-price"><i>Rent: </i>' + priceToString(item.priceRentMonthly) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p><p class="object-price"><i>Sale: </i>' + priceToString(item.priceSale) + ' ' + '<b>' + getCurrencySymbol() + '</b>' + '</p>' : '')))) + '</div>\
										</div>');
						} else {
							$container.append('<div class="sisea-result slide offset">\
														<a href="#!p=' + item.id + '-' + item.alias + '&s=pano' + item.panoId + '&lang=' + cultureKey + '" title="' + item.pagetitle + '" style="background-image: url(' + item.img + '?' + randomHash + ');" class="object" data-isfolder="' + item.isfolder + '" data-id="' + item.id + '">' + ($.cookie('object_' + item.id) ? '' : '<div class="new"></div>') + '<p><span class="title-district">'+ item.districtTitle +'</span> ' + item.pagetitle + (item.longtitletwo.length > 3 ? '<br>' + item.longtitletwo : '') + '</p></a>\
									</div>');
						}
					}
				}
...
function getPano(itemId) {
    ...
    var panoLocation = $('#panoLocation');
	    ...
	    $.ajax({
        url: 'https://360crimea.ru/api/get-object',
        data: {
        ...
        success: function(data) {
            ...
            if (published == 0) {
                ...
                if (data.template == 3) {
                    ...
                    panoLocation.empty().html(translation.Site_Country(cultureKey) + '<br><span>' + data.title + '</span>');
                } else {
                    $('title').text(data.seoTitle);
                    $("meta[name='title']").attr('content', data.seoTitle);
                    $("meta[property='og:title']").attr('content', data.seoTitle);
                    panoLocation.empty().html(translation.Site_Country(cultureKey) + '<br><span>' + data.location + '<br>' + data.longtitle + (data.isRealEstate == true ? ' ID: ' + objectId : '') + '</span>');
                }
                if (data.isSample) {
...
function getPanoByCategoryId(itemId) {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-object-by-category',
        data: {
            itemId: itemId,
            lang: cultureKey,
            categoryId: categoryId,
        ...
        success: function(data) {
            ...
            for (key in itemObject) {
                ...
                if (data.template == 3) {
                    panoLocation.empty().html(translation.Site_Country(cultureKey) + '<br><span>' + item.title + '</span>');
                } else {
                    panoLocation.empty().html(translation.Site_Country(cultureKey) + '<br><span>' + item.location + '<br>' + item.longtitle + (item.isRealEstate == true ? ' ID: ' + item.id : '') + '</span>');
                }
            }
...
function getAboutInfo(isContacts) {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-about-info',
        data: {
            lang: cultureKey,
			rand: getRandom()
        },
            ...
            if (!isContacts) {
                ...
                for (key in pages) {
                    ...
                    if (page.alias == 'project-news-i-updates') {
                        ...
                        // if (counter == 3) {
                        //     $('.block-3 .about-content-body').append('<div id="ourLocation"><iframe class="location-iframe" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1706.0730061400254!2d24.752284774602863!3d59.43517334792899!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x469293603d53eb33%3A0x3781d82e99e20d36!2zRXN0b25pYSBwdWllc3RlZSA1LTMwOUIsIDEwMTQzIFRhbGxpbm4sINCt0YHRgtC-0L3QuNGP!5e0!3m2!1sru!2s!4v1526375438602" frameborder="0" style="border:0"></iframe><div>');
                        // }
                    }
                    counter++;
                };
...
function OnLoadNews(offset) {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/loading-news',
        data: {
            lang: cultureKey,
            offset: offset,
			rand: getRandom()
...
function getAboutObjectInfo(showFeedBack){
    ...
    var aboutContent = $('.about-object-content');
		var panoWindow = document.getElementById('krpanoSWFObject');
		var scene = panoWindow.get("xml.scene");
		var panoId = scene.replace('pano', '');
    var itemId = getUrlVars()["p"];
    if (typeof itemId === 'undefined' || itemId === null) {
        itemId = 1;
    } else {
        itemId = getUrlVars()["p"].split('-')[0];
    }
    $('.about-object-modal').queue(function() {
		$('.about-object-modal').animate({right: 0}, 500);
		$.ajax({
        url: 'https://360crimea.ru/api/get-about-object-info',
        data: {
            itemId: itemId,
						panoId: panoId,
            lang: cultureKey,
...
function getInnerContent() {
    ...
    $.ajax({
        url: 'https://360crimea.ru/api/get-default-object',
        data: {
            itemId: itemId,
            lang: cultureKey,
			rand: getRandom()
...
function testDriveCall(link) {
	$('.drive_modal iframe').attr('src', link);
	$('.drive_modal').fadeIn(500);
}
function OnCloseDriveModal() {
	$('.drive_modal iframe').attr('src', '');
	$('.drive_modal').fadeOut(500);
}
function OnDocumentReady() {
    if(top.location != self.location){
        $('#category-search-btn').hide();
    ...
    $(".feed-back-form form").submit(function() {
        ...
        if (error) {
            ...
            window.location.href = 'https://360crimea.ru/message-sent?redirect=' + oldHref;
        }
    });
    ...
    $(document).on('click', '.help-content-close-btn', OnCloseHelpModal);
		$(document).on('click', '.drive_modal_close_btn', OnCloseDriveModal);
    ...
    $(document).on('click', '.category-list a', function() {
        ...
        if ($this.hasClass('object') && parseInt($this.data('isfolder')) == 0) {
            $this.find('.new').remove();
            if (isMobile.any()) {
                OnCloseSearchPanel(true);
            }
        } else {
						if(parseInt($this.data('isfolder')) == 1){
							GetCompanies($this.data('id'));
							$('.search-logo').attr('data-iscompanies', 1);
						}
						else{
							OnCategorySearch($this.data('search-id'), $this.data('id'), lastLevel, '', false);
							$('.search-logo').attr('data-iscompanies', 0);
						}
        }
    });
    ...
    $(document).on('click', '.back', function() {
        ...
        if ($(this).attr('data-id') == '0') {
            ...
            if (parseInt($this.attr('data-id')) == 4) {
                GetTopObjects();
            } else {
                if(parseInt($this.attr('data-id')) == 226){
									$('.search-logo').attr('data-iscompanies', 0);
									OnCategorySearch($this.attr('data-search-id'), $this.attr('data-id'), false, '', false);
								}
								else {
    ...
    if (isMobile.any()) {
        $('.search-time').show();
        $("body").append($('<link rel="stylesheet" href="https://360crimea.ru/assets/css/mobile_adaptation.css? ' + getRandom() + '" type="text/css" />'));
    }
...
$(window).load(function() {
    ...
    CSSLoad('https://360crimea.ru/assets/css/owl.carousel.css');
    CSSLoad('https://360crimea.ru/assets/css/bootstrap.css');
    CSSLoad('https://360crimea.ru/assets/css/jquery.scrollbar.css');
    CSSLoad('https://360crimea.ru/assets/css/main.css?' + getRandom());
	getBtnTranslations(cultureKey);
    if ((typeof categoryId === 'undefined' || categoryId === null) && (parseInt($('.search-logo').attr('data-id')) != 41)) {
        GetTopObjects();
    } else {
        OnSearchPanelShow();
        $('.back').attr('data-id', 4).attr('data-search-id', 4);
	...
	if(typeof categoryId != 'undefined' || parseInt(panoId) == 160){
			searchTimeout();
	}
